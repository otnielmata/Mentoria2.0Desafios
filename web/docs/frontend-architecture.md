...
A consulta de configuracoes iniciais segue o fluxo:

```text
src/app/configuracoes/page.js
  -> getConfigurations em src/controllers/configurations.controller.js
  -> toConfigurationsDto em src/models/configurations.model.js
  -> getConfigurationsRequest em src/services/configurations.service.js
  -> GET /api/configuracoes
```

A rota `/configuracoes` e restrita a professor/admin pelo `AuthGuard`. A tela exibe parametros iniciais em modo somente leitura, mostra a visibilidade do ranking geral para alunos quando a API retornar essa informacao e apresenta estado vazio util quando nao houver configuracoes disponiveis. O front-end nao habilita edicao se a API nao suportar e remove parametros com conteudo sensivel antes de renderizar.
